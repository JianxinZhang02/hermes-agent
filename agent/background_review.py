...
_SKILL_ANTI_OVERFIT_GUARDRAILS = (
    ...
    "4. Check the current skill first. If it already expresses the same method, "
    "do not patch it. A newly observed question subtype is not a new method. Make "
    "at most one minimal logical update and prefer replacement, consolidation or "
    "deletion over net growth.\n"
    "5. Keep SKILL.md at or below 800 words. Do not create question catalogs, "
    "answer banks, trajectories or session case files. Support files are only for "
    "stable, independently reusable material; ordinary QA, classification, and "
    "extraction skills need no supporting file.\n"
    "6. Any skill created or materially extended must retain an exact "
    "`## WHEN TO USE` section, an executable numbered procedure, and a "
    "`## LIMITATIONS` or equivalent out-of-scope section. Do not add boilerplate "
    "merely to satisfy this shape; if the method cannot support it, do not save it.\n"
