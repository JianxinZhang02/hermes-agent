...
def init_agent(
    ...
    agent._builtin_memory_provider = None
    _active_memory_provider = None
    if not skip_memory or _memory_toolset_requested:
        try:
            ...
            if agent._memory_store is not None:
                agent._builtin_memory_provider = BuiltinMemoryProvider(
                    agent._memory_store,
                    memory_enabled=agent._memory_enabled,
                    user_profile_enabled=agent._user_profile_enabled,
                )
            # ``skip_memory`` historically disables provider lifecycle and
            # prompt injection even when the core memory toolset is requested.
            # Keep its tool usable through the adapter without constructing a
            # manager; normal operation registers the same adapter below.
            if skip_memory:
                if agent._builtin_memory_provider is not None:
                    agent._builtin_memory_provider.initialize(agent.session_id)
                agent._memory_manager = None
            else:
                agent._memory_manager = _MemoryManager()
                if agent._builtin_memory_provider is not None:
                    agent._memory_manager.add_provider(agent._builtin_memory_provider)
            _mem_provider_name = mem_config.get("provider", "") if mem_config else ""

            if not skip_memory and _mem_provider_name and _mem_provider_name.strip():
                from plugins.memory import load_memory_provider as _load_mem
                _mp = _load_mem(_mem_provider_name)
                if _mp and _mp.is_available():
                    agent._memory_manager.add_provider(_mp)
                    _active_memory_provider = _mp
                else:
                    _ra().logger.debug("Memory provider '%s' not found or not available", _mem_provider_name)

            if agent._memory_manager is not None and agent._memory_manager.providers:
                _init_kwargs = {
                ...
                if _active_memory_provider is not None:
    ...
    # Keep indexed resources separate from conversational memory.  A legacy
    # dual-capability provider is a non-owning KB view: MemoryManager remains
    # its sole lifecycle owner.
    agent._knowledge_base_manager = None
    try:
        from agent.knowledge_base_manager import KnowledgeBaseManager
        from agent.knowledge_provider import KnowledgeBaseProvider
        from agent.retrieval_scope import ProviderScope

        _knowledge_cfg = _agent_cfg.get("knowledge", {}) or {}
        _cwd = os.getenv("TERMINAL_CWD") or os.getcwd()
        _profile_name = locals().get("_profile", "")
        _scope = ProviderScope(
            user_id=str(agent._user_id or agent._user_id_alt or ""),
            agent_id=str(_profile_name),
            session_id=str(agent.session_id or ""),
            project_id=os.path.normcase(os.path.abspath(_cwd)) if _cwd else "",
            workspace=os.path.basename(os.path.abspath(_cwd)) if _cwd else "",
            platform=str(platform or "cli"),
            profile=str(_profile_name),
            hermes_home=str(get_hermes_home()),
        )
        if agent._memory_manager is not None:
            agent._memory_manager.scope = _scope
        _kb_manager = KnowledgeBaseManager(
            enabled=bool(_knowledge_cfg.get("enabled", True)),
            scope=_scope,
        )
        _kb_manager.max_context_chars = max(
            1,
            int(_knowledge_cfg.get("max_context_chars", 12000)),
        )
        if isinstance(_active_memory_provider, KnowledgeBaseProvider):
            _kb_manager.add_provider(_active_memory_provider, owns_lifecycle=False)
        if _kb_manager.provider is not None:
            agent._knowledge_base_manager = _kb_manager
    except Exception as _kb_exc:
        _ra().logger.warning("Knowledge provider init failed: %s", _kb_exc)

    from agent.memory_manager import inject_memory_provider_tools as _inject_memory_provider_tools
    _inject_memory_provider_tools(agent)
    from agent.knowledge_base_manager import inject_knowledge_provider_tools as _inject_knowledge_provider_tools
    _inject_knowledge_provider_tools(agent)
