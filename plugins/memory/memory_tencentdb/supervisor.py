...
def _startup_singleflight(host: str, port: int) -> Iterator[None]:
    ...
    lock_path = os.path.join(
        tempfile.gettempdir(), f"memory-tencentdb-gateway-{key}.lock"
    )
    with thread_lock:
        lock_file = None
        try:
...
def _signal_process_group(proc: subprocess.Popen, sig: int) -> bool:
    """Signal a POSIX process group; return False on unsupported platforms."""
...
class GatewaySupervisor:
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: Full HTTP(S) endpoint. When provided it overrides host
                and port for client traffic.
            ...
            api_key: Optional Gateway Bearer token used by the **client**
                ...
                ``None`` / empty uses the upstream local compatibility token.
            service_id: Value for the ``x-tdai-service-id`` memory-space header.
            request_timeout: Per-request HTTP timeout in seconds.
            child_env: Gateway-only environment values (config/data/LLM).
            log_dir: Optional profile-scoped sidecar log directory.
        """
        endpoint = (base_url or f"http://{host}:{port}").strip().rstrip("/")
        parsed = urlparse(endpoint)
        if parsed.scheme not in {"http", "https"} or not parsed.hostname:
            raise ValueError(f"Invalid memory-tencentdb Gateway endpoint: {endpoint!r}")
        ...
        self._configured_log_dir = (log_dir or "").strip()
        ...
        self._gateway_cmd = gateway_cmd or os.environ.get(
    ...
    def ensure_running(self) -> bool:
        ...
        if self.is_running():
            logger.info(
                "memory-tencentdb Gateway already running at %s", self._base_url
            )
            return True
    ...
    def _wait_for_health(self) -> bool:
        ...
        while time.monotonic() - start < HEALTH_CHECK_MAX_WAIT:
            if self._shutdown_requested:
                logger.info(
                    "memory-tencentdb Gateway startup wait cancelled by shutdown"
                )
                return False
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "memory-tencentdb Gateway did not exit in 10s, sending SIGKILL"
                )
                sigkill = getattr(signal, "SIGKILL", signal.SIGTERM)
                if not _signal_process_group(proc, sigkill):
                    proc.kill()
                proc.wait(timeout=5)
        except Exception as e:
